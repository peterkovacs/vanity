...
$(document).ready(function() {
  ...
  $(".experiment button.reset").on("click", function() {
  ...
  $(".experiment button.finish").on("click", function() {
    var link = $(this);
  ...
  $(".experiment a.button.reset").on("click", function() {
    if (confirm('Are you sure you want to reset the experiment? This will clear all collected data so far and restart the experiment from scratch. This cannot be undone.')){
      var link = $(this);
      $.ajax({
        data: 'authenticity_token=' + encodeURIComponent(document.auth_token),
  ...
  $(".experiment a.button.versions").on("click", function() {
    var experiment = $(this).attr("data-experiment");
    $(".version", document.getElementById(experiment)).toggleClass( "hidden" ).fadeIn(750);
  });
});
